<script>
  ...
  async function fetchSources(){
    sources = await PaceHelper.fetchSourceStatus();
...
<div class="w-full bg-gray-100 shadow-sm border-4 border-dotted">
  ...
  {:then}
    ...
    {:else if sources == null}
      <div class="flex flex-col pl-4 pb-4">
        <h1 class="font-semibold text-xl">There was an error while fetching data sources.</h1>
        <p class="font-base text-md pt-3">
          Please verify that the Pace server is online and functioning.
        </p>
      </div>
    {:else}
      <div class="flex border-b-[2px] m-4 pb-2 border-gray-300">
